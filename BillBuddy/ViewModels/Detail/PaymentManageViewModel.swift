...
final class PaymentManageViewModel: ObservableObject {
    ...
    func addPayment(paymentStore: PaymentServiceOrigin, settlementExpensesStore: SettlementExpensesStore, locationManager: LocationManager, notificationStore: NotificationService) {
        let newPayment =
    ...
    func mainAddPayment(paymentStore: PaymentServiceOrigin, settlementExpensesStore: SettlementExpensesStore, locationManager: LocationManager, notificationStore: NotificationService, userTravelStore: UserTravelStore) {
        let newPayment =
        Payment(type: selectedCategory ?? .etc, content: expandDetails, payment: Int(priceString) ?? 0, address: Payment.Address(address: locationManager.selectedAddress, latitude: locationManager.selectedLatitude, longitude: locationManager.selectedLongitude), participants: participants, paymentDate: paymentDate.timeIntervalSince1970)
    ...
    func editPayment(paymentStore: PaymentServiceOrigin, settlementExpensesStore: SettlementExpensesStore, locationManager: LocationManager, notificationStore: NotificationService) {
        if let payment = payment {
